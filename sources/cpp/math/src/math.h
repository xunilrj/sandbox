...
{
    return vec3f{f * b.x, f * b.y, f * b.z};
}
...
ray_triangle_result<typename TVec::type> intersection_ray_triangle(
    ...
    const TVec &a, const TVec &b, const TVec &c)
{
    ...
    if (!is_between(u, 0, 1 - v))
        return {0};

    auto t = edge2.dot(qvec) * inv_det;

    return ray_triangle_result<typename TVec::type>{true, t, u, v};
}
