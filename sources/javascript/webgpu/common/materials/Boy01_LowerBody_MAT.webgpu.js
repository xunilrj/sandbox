...
{
    const vs = device.createShaderModule({ code: await loadSPV("http://localhost:5000/shaders/Boy01_LowerBody_MAT.vert.spv") });
    const fs = device.createShaderModule({ code: await loadSPV("http://localhost:5000/shaders/Boy01_LowerBody_MAT.frag.spv") });
    const sampler = device.createSampler({ minFilter: "linear", magFilter: "linear" });
    const tex_CHANNEL0 = await createTexture(device, "http://localhost:5000/textures/Boy_LowerBody_diffuse");
