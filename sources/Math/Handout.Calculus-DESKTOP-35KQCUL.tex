...
\begin{theorem}[Derivative of $ln(x)$]\label{theorems:calculus:derivatives:lnx}
	\begin{align*}
	f(x) &= ln(x)\\
	f'(x)&=1/x
	\end{align*}
\end{theorem}
\begin{center}
	\plotffprimerange {ln(x))}{1/x}{-10}{10}{-1}{1}
\end{center}
\begin{proof}
	\begin{align*}
	f(x) &= ln(x) \\
	f'(x) &= \frac{d}{dx}ln(x)\\
	f'(x) &= \frac{d}{dx} \int_1^x{\frac{1}{t}dt}\\
	f'(x) &= \frac{1}{x} && \text{use (\ref{theorems:calculus:fundamental})}\\
	\end{align*}
\end{proof}

[Fundamental Theorem of Calculus]\label{theorems:calculus:fundamental}
\end{document}