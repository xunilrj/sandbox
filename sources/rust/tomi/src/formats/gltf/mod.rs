...
pub fn push_sampler_to_animation(anim: &mut JsonValue, sampler: JsonValue) -> usize {
...
pub fn push_channel_to_animation(anim: &mut JsonValue, channels: JsonValue) -> usize {
...
pub fn push_skin(gltf: &mut JsonValue, skin: JsonValue) -> usize {
    if !gltf["skins"].is_array() {
        gltf["skins"] = json::array![];
    }

    let _ = gltf["skins"].push(skin);
    gltf["skins"].len() - 1
}

pub fn push_animation(gltf: &mut JsonValue, animation: JsonValue) -> usize {
    if !gltf["animations"].is_array() {
        gltf["animations"] = json::array![];
    }

    let _ = gltf["animations"].push(animation);
    gltf["animations"].len() - 1
}
...
pub fn push_node_to_scene(scene: &mut JsonValue, node: usize) {
    if scene["nodes"].len() == 0 {
        scene["nodes"] = json::array![];
    }

    let _ = scene["nodes"].push(node);
}
