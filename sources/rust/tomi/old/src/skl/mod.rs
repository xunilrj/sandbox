...
use crate::parser::{NomSlice, SymbolExtension};
...
pub fn parse_skl<P: AsRef<Path>>(path: P) -> SklFile {
    ...
    let properties = input.read_properties(&map);
    debug!("{:?}", properties);
    ...
    for i in 0..qty {
        ...
        let k = input.parse_le_u64("Bone Parent ID").print_name(&map);
        let _ = input.parse_le_u32("Zero");
        ends.push(k);
        ...
        let bone = Bone {
            name: map.get_mapping(ends[0]),
            start: ends[0],
            parent_bone_id: ends[1],
