...
use crate::parser::{self, NomSlice, SymbolExtension};
...
pub fn convert(path: impl AsRef<Path>, mesh: String, skl: String, output: impl AsRef<Path>) {
    ...
    let map = crate::checksum_mapping::ChecksumMap::new();
    ...
    let file_properties = input.read_properties(&map);
    ...
    for _ in 0..qty_track_types {
        let hash = input.parse_le_u64_with_debug("track type hash", |hash| match hash {
            0x1019453EB19C1ABD => "Keyframed track of quaternion".to_string(),
            0xCECACE3A835CB7EE => "Single Quaternion".to_string(),
            0x5D3E9FC6FA9369BF => "Keyframed track of transform".to_string(),
            0xF6F394AF6E4003AD => "Keyframed track of vector3".to_string(),
            0xFC6597EB1FE5458E => "Compressed transforms".to_string(),
            0xC1E84D6FF72CE80 => "Single Transform".to_string(),
            x => map.get_mapping(x).unwrap_or("?".to_string()),
        });
        ...
        match track_type.hash {
                        ...
                        loop {
                            ...
                            for _ in 0..qty_samples {
                                ...
                                let frame = AnimatedTrackFrame {
                                    time,
                                    t: TransformType::Global,
                                    rotation: [last_rot[0], last_rot[1], last_rot[2], last_rot[3]],
                                    translation: [
                                        last_translation[0] * 0.01,
                                        last_translation[1] * 0.01,
                                        last_translation[2] * 0.01,
                                    ],
                                };
                                debug!("{:?}", frame);
            ...
            0x1019453EB19C1ABD => {
                ...
                for _ in 0..track_type.qty {
                    ...
                    let _ = input.parse_le_u32("?");
                    let _ = input.parse_le_u32("?");
                    let bone_id = input.parse_le_u64("bone id").print_name(&map);
            ...
            0x5D3E9FC6FA9369BF => {
                ...
                for _ in 0..track_type.qty {
                    ...
                    let boneid = input.parse_le_u64("boneid").print_name(&map);
                    let _ = input.parse_le_u32("?");
            ...
            0xF6F394AF6E4003AD => {
                ...
                for _ in 0..track_type.qty {
                    ...
                    let _ = input.parse_le_u32("?");
                    let _ = input.parse_le_u32("?");
                    let _ = input.parse_le_u64("?").print_name(&map);
                    let _ = input.parse_le_u32("?");
                    let _ = input.parse_le_u32("?");
    ...
    debug!("Bone IDs");
    for i in 0..qty_tracks {
        let k = input.parse_le_u64("Bone ID").print_name(&map);
        let _ = input.parse_le_u32("");
    ...
    // 9062916324265328658 = root parent
    let mut tracks = tracks
        .iter()
        .filter(|x| x.bone_id != 9062916324265328658)
        .cloned()
        .collect::<Vec<_>>();
    ...
    let mut skl_bones_bone_id = BTreeSet::new();
    for bone in skl.bones.iter() {
        skl_bones_bone_id.insert(bone.start);
    }

    let mut bone_id_to_track_idx = HashMap::new();
    for (i, bone) in tracks.iter().enumerate() {
        if !skl_bones_bone_id.contains(&bone.bone_id) {
            panic!(
                "Bone not found: {:?} {:?}",
                map.get_mapping(bone.bone_id),
                bone
            );
        }
        let r = bone_id_to_track_idx.insert(bone.bone_id, i);
        assert!(r.is_none());
    }

    // println!("{} == {}", bone_id_to_track_idx.len(), skl.bones.len());
    // assert!(bone_id_to_track_idx.len() == skl.bones.len());

    let bones_ids = gltf["skins"][0]["joints"]
        .members()
        .map(|x| {
            gltf["nodes"][x.as_usize().unwrap()]["metadata"]["id"]
                .as_str()
                .unwrap()
                .parse::<u64>()
                .unwrap()
    ...
    // println!("{} == {}", bones_ids.len(), skl.bones.len());
    // assert!(bones_ids.len() == skl.bones.len());
    ...
    // Convert compressed global transformation to local
    #[derive(Copy, Clone)]
    struct Global2Local {
        global: Option<Mat4>,
        local: Mat4,
    ...
    for track in tracks.iter() {
        let bone_idx = bone_id_to_track_idx[&track.bone_id];
        for frame in track.frames.iter() {
            let t = format!("{}", frame.time);
            let pose = times
                .entry(t)
                .or_insert_with(|| vec![Global2Local::new(); skl.bones.len() + 1]);
            pose[bone_idx].global = Some(Mat4::from_scale_rotation_translation(
                vec3(1.0, 1.0, 1.0),
                Quat::from_array(frame.rotation_as_f32()),
                Vec3::from_slice(&frame.translation_as_f32()[..]),
            ));
        }
    }
    let mut last_global = vec![Mat4::IDENTITY; skl.bones.len() + 1];
    for (t, bones) in times.iter_mut() {
        for (i, bone) in bones.iter().enumerate() {
            if let Some(global) = bone.global {
                last_global[i] = global;
            }
        }

        for bone in skl.bones.iter() {
            let idx = bone_id_to_track_idx[&bone.start];
            let inv_parent = bone
                .parent
                .map(|p| last_global[p].clone())
                .unwrap_or(Mat4::IDENTITY)
                .inverse();
            bones[idx].local = inv_parent * last_global[idx];
        }
    }
    for track in tracks.iter_mut() {
        ...
        for frame in track.frames.iter_mut() {
            let t = format!("{}", frame.time);
            let g2l = &times[&t][bone_idx];
            let (s, q, t) = g2l.local.to_scale_rotation_translation();
            frame.rotation = [q.x as f64, q.y as f64, q.z as f64, q.w as f64];
            frame.translation = [t.x as f64, t.y as f64, t.z as f64];
        }
    }
    ...
    for bone_id in bones_ids.iter() {
        let track = &tracks[bone_id_to_track_idx[bone_id]];
    ...
    for bone_id in bones_ids.iter() {
        ...
        let b: Vec<u8> = track
            .frames
            .iter()
            .scan(Quat::default(), |acc, frame| Some(frame.rotation_as_f32()))
            .flat_map(|x| x)
