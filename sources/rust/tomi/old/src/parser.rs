...
impl<'a> NomSlice<'a> {
    ...
    pub fn read_properties(&mut self, map: &ChecksumMap) -> HashMap<String, u32> {
        let qty = self.parse_le_u32("qty of properties");
        self.read_n_properties(qty as usize, map)
    }

    pub fn read_n_properties(&mut self, n: usize, mapping: &ChecksumMap) -> HashMap<String, u32> {
...
pub trait SymbolExtension {
    fn print_name(self, mapping: &ChecksumMap) -> Self;
}

impl SymbolExtension for u64 {
    fn print_name(self, mapping: &ChecksumMap) -> Self {
        debug!("mapping: {:?}", mapping.get_mapping(self));
        self
    }
}
