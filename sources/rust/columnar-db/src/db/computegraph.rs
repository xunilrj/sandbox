...
impl<'a> ComputeGraph<'a> {
    ...
    pub fn new_node(&mut self, def: &super::computegrafdef::ComputeTaskDefinitionTypes) {
        use super::computegrafdef::ComputeTaskDefinitionFolds::*;
        use super::computegrafdef::ComputeTaskDefinitionSources::*;
        use super::computegrafdef::ComputeTaskDefinitionTypes::*;
        match def {
            Source(ReadColumn { column }) => {
                let stream = self.catalog.col_read_all(column, 16 * 1024);
