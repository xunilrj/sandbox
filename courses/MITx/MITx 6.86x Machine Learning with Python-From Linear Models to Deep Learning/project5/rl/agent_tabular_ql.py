...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if(e[0] <= epsilon):
        ...
        object_index = np.argmax( np.max(q_func[state_1, state_2,:,:], axis=0) )
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    next = 0
    if(not terminal):
        bestQ = np.max(q_func[next_state_1, next_state_2,:,:])
        next = GAMMA*bestQ
    old = (1-ALPHA)*q_func[current_state_1, current_state_2,  action_index, object_index]
    new = ALPHA*(reward+next)
    q_func[current_state_1, current_state_2,
           action_index, object_index] = old + new
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        current_action, current_object = epsilon_greedy(
            current_room, current_quest, q_func, epsilon)
        ...
        if for_training:
            ...
            tabular_q_learning(q_func,
                current_room, current_quest,
                current_action, current_object,
                reward, next_room, current_quest,
                terminal)
        ...
        next_quest_desc = next_quest_desc
