...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        # YOUR CODE GOES HERE
        #rospy.logdebug("%s", map(lambda x: x.name, joints))
        #rospy.logdebug("%s", joint_values)
        jv = dict(zip(joint_values.name, joint_values.position))
        lj = list(zip(link_names, joints))

        def getJointValue(name, jv = jv):
            if(name in jv ): return jv[name]
            else: return 0

        T = [tf.transformations.identity_matrix()]
        def apply(linkName, translation, rpy, jointAxis, jointValue, T = T):
            T[0] = numpy.dot(numpy.dot(T[0],
                tf.transformations.translation_matrix(translation)),
                tf.transformations.quaternion_matrix(
                    tf.transformations.quaternion_from_euler(*rpy)
                )
            )
            ...
            if(jointAxis != None):
                T[0] = numpy.dot(T[0],
                tf.transformations.quaternion_matrix(
                    tf.transformations.quaternion_about_axis(jointValue, jointAxis)
                )
            )

            return convert_to_message(T[0], linkName, "world_link")        
        
        all_transforms.transforms = [
            apply(linkName, joint.origin.xyz, joint.origin.rpy, joint.axis, getJointValue(joint.name))
            for linkName,joint
            in lj]
...
if __name__ == '__main__':
    rospy.init_node('fwk', anonymous=True, log_level=rospy.DEBUG)
