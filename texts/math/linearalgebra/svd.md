...
3 - the blackboard. Now you will get a whole  course in Linear Algebra. It is very likely you will found yourself in this situation.
...
First thing we will do is install and use octave and some packages. This can be very easily done with:
...
Now we will start to develop and evolve our script. First thing is we will download an image. We will treat an image as a matrix where each element of the matrix will be the intensity of gray in the image.
...
The magic values 0.3, 0.63 and 0.07 used here to mix the R, G and B colors exist because not every color has the same intensity. That is why you can "see" a lot of more red in some diagrams and maps.
...
OK, but this begs the question "why would I exchange my image, in form a matrix, to three others matrices that have no apparent semantic meaning"?

And this is exactly the point, this three matrices DO have a semantic meaning. To avoid jumping into the mathematical explanation that will also appear meaningless, we will give a practical use to these three matrices.

Out first step is to break the multiplication

    [U, S, V] = svd (grayImg);

into steps. We all learned that matrix multiplication is done "dot-producting" the rows of the "left operand" with the columns of the "right-operand".
...
The biggest difference here is that "M1(:,1)*M2(1,:)", the first column of the left operant times the first row of the right operand, generates a matrix and not a single value. So, every step you get a full matrix and sum with the current value you have.
...
No going back to the multiplication 

    U*S*Vt

but now doing it by Rank-1 update (like we did above) we will see something VERY interesting. Let first do just for the first column-row.

    img1 = U(:,1)*S(1,1)*Vt(1,:);
    imwrite(img1, "img1.jpg");
    size(img1) == size(img(:,:,1))

The last line confirms that this multiplication has the exact same size as our original image. As expected by the Rank-1 update process. Let us see the image:

![Row-Column Matrix Multiplication](./img1.jpg)

Ok, far from interesting. But this process can be extended for all column and we know that at the last column our matrix will be exactly as our original image. The question is what happens what intermediary steps. That is exatly what the following code does:

We will accumulate and save the intermediary steps. Hoping to see something interesting.
...
OK. Now we did 500 steps and generated 500 images. At each image we use more and more percentage of the three matrices that the SVD algorithms gave us. To understand what happens let us generate a gif from this images.
...
Ok! This IS interesting. There are two very important details.
    1 - We are increasingly "closer" to the final image coming from a totally white image;
    2 - It is very hard to see what the last steps are doing to tis approximation.
...
Ignoring the light intensity, it is even hard to se any difference between the images. But the important fact to realize is that the image is named "img500". This means that we used only 500 column, of 1296 or 38.58% of the columns.

If we go deeper we will see that we only used:
...
The original image has one value for each pixel and has size of 1296 * 1936 = 2509056
This means:
...
Freeze, punk!! Have we created a compression algorithm? Yes! We create a very similar image using only 64% of the data. Although we should remember that this is a lossy compression. The "img500.jpg" has "less" information than the original "img.jpg".
...
This means that "u500*s500*vt500" is an approximation (using 30% less data) than the original "img". In Linear Algebra lingo, this compression is called "dimensionality reduction" because you are using just 500 dimensions, instead of 1296. But in this process of reducing the dimensionality of our problem, we cannot choose any 500 dimensions of the 1296. Otherwise we could end up with the 500-worst dimensions.

This brings a very interesting point. What would be the 500-worst dimensions? And what would be the 500-best dimensions? If we want to use just 500 dimensions out of 1296. 

Well, if we know that "u500*s500*vt500" is an approximation to "img" this means that we know that these two matrices are not equal. If, on top of that, we increase the analysis to have somehow a sense of "distance" between two matrices. We could, at least in theory, choose all 500-dimension combinationn of 1296 and choose the one with the minimum "distance". That would be the best 500 dimensions.

Looks like a easy task. Lets remember from Combinatorics that this is "1296 choose 500" and the result is...
...
Err.. Well... we will have to have another method... and this method is... the SVD. The best 1-dimension approximation of the "img" is "u1*s1*vt1". Exactly our first image. Let us take a look at it.

![Step-by-step of SVD](./img1.jpg)

Not very impressive, but what would you expect with using just 1 of 1296 dimensions? This first dimension corresponds to the first column of U, the first item in the diagonal of S and the first row of Vt. They are the "principal singular vectors" and the "principal singular value" of the "img" matrix.

If we follow the the n-th column using this method, we get the n most important singular vector and singular values. We can call them the principal components of our original matrix, in our case, our image.